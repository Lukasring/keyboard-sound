const pressedkeyDOM = document.getElementById("key-display")!;
const octaveSelectorDOM = document.getElementById(
  "octave"
) as HTMLSelectElement;
const waveTypeSelectorDOM = document.getElementById(
  "wave-type"
) as HTMLSelectElement;
...
function createBeep(noteFreq: number) {
  const context = new AudioContext();
  const oscillator = context.createOscillator();
  const gain = context.createGain();
  oscillator.type = waveTypeSelectorDOM.value as OscillatorType;
...
function keySoundMap(keyCode: string, octave: number) {
  const keyMap: KeyMap = {
    KeyA: notes["C"][octave],
    KeyS: notes["C#"][octave],
    KeyD: notes["D"][octave],
  ...
  const freq: number = keyMap[keyCode];
  if (!freq) {
